...
module.exports.welcome = async (iswel, isleft, mane, anu) => {
  try {
    const metadata = await mane.groupMetadata(anu.id);
    const participants = anu.participants;
    const groupName = metadata.subject;
    const groupDesc = metadata.desc;
    for (let num of participants) {
      try {
        pp_user = await mane.profilePictureUrl(num, 'image');
      } catch {
        pp_user = 'https://telegra.ph/file/c3f3d2c2548cbefef1604.jpg';
      }
      try {
        ppgroup = await mane.profilePictureUrl(anu.id, 'image');
      } catch {
      ...
      if (anu.action === 'add' && (iswel || setting.auto_welcomeMsg)) {
        ...
        if (isSetLeft(anu.id, set_left_db)) {
          ...
          const full_pesan = replace_pesan.replace(/@group/gi, groupName).replace(/@desc/gi, groupDesc);
          mane.sendMessage(anu.id, { image: { url: pp_user }, mentions: [num], caption: `${full_pesan}` });
        } else {
          mane.sendMessage(anu.id, { text: `ʙᴀɪʙᴀɪ ᴋᴀᴋ @${num.split("@")[0]} sᴇᴍᴏɢᴀ ᴛᴇɴᴀɴɢ ᴅɪ ᴀʟᴀᴍ sᴀɴᴀ`, mentions: [num]});
        }
      }
      else if (anu.action === 'promote') {
        mane.sendMessage(anu.id, {
