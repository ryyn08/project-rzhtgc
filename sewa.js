...
async function remindSewa(mane, sewa) {
    for (let x of sewa) {
        if (x.expired !== "PERMANENT") {
            let remaining = x.expired - Date.now()
            if (remaining <= 24 * 60 * 60 * 1000 && remaining > 23 * 60 * 60 * 1000) {
                await mane.sendMessage(x.id, { text: "⚠️ Sewa bot akan berakhir dalam 1 hari!" })
            }
            if (remaining <= 60 * 60 * 1000 && remaining > 59 * 60 * 1000) {
                await mane.sendMessage(x.id, { text: "⚠️ Sewa bot akan berakhir dalam 1 jam!" })
            }
        }
    }
}
...
async function getGcName(id) {
    try {
        let metadata = await global.mane.groupMetadata(id)
