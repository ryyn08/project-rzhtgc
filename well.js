...
module.exports.welcome = async (iswel, isleft, manenu) => {
  try {
    const metadata = await hydmaneMetadata(anu.id);
    const participants = anu.participants;
    const groupName = metadata.subject;
    const groupDesc = metadata.desc;
    for (let num of participants) {
      try {
        pp_user = await mane.maneictureUrl(num, 'image');
      } catch {
        pp_user = 'https://telegra.ph/file/c3f3d2c2548cbefef1604.jpg';
      }
      try {
        ppgroup = await mane.promaneureUrl(anu.id, 'image');
      } catch {
      ...
      if (anu.action === 'add' && (iswel || setting.auto_welcomeMsg)) {
        ...
        if (isSetLeft(anu.id, set_left_db)) {
          ...
          const full_pesan = replace_pesan.replace(/@group/gi, groupName).replace(/@desc/gi, groupDesc);
          mane.sendMessage(mane{ image: { url: pp_user }, mentions: [num], caption: `${full_pesan}` });
        } else {
          mane.sendMessage(anumaneext: `ʙᴀɪʙᴀɪ ᴋᴀᴋ @${num.split('@')[0]} sᴇᴍᴏɢᴀ ᴛᴇɴᴀɴɢ ᴅɪ ᴀʟᴀᴍ sᴀɴᴀ`, });
        }
      }
      else if (anu.action === 'promote') {
        mane.sendMessage(anu.idmane      text: `ʜᴇʏ ᴋᴀᴍᴜ! @${num.split('@')[0]}\nᴘᴀɴɢᴋᴀᴛ ᴋᴀᴍᴜ ᴅɪ ɢʀᴜᴘ ${groupName} ɴᴀɪᴋ ᴍᴇɴᴊᴀᴅɪ ᴀᴅᴍɪɴ 🤪`,
        });
      }
      else if (anu.action === 'demote') {
        mane.sendMessage(anu.id, {mane   text: `ʜᴇʏ ᴋᴀᴍᴜ! @${num.split('@')[0]}\nᴘᴀɴɢᴋᴀᴛ ᴋᴀᴍᴜ ᴅɪ ɢʀᴜᴘ ${groupName} ᴛᴜʀᴜɴ ᴍᴇɴᴊᴀᴅɪ ᴀɴɢɢᴏᴛᴀ 👀`,
        });
      }
    }
  } catch (err) {
