...
async function pagi(mane, x, data) {
    ...
    return await mane.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
...
async function voting(mane, x, data) {
    let row = [];
    let ment = [];
...
async function malam(mane, x, data) {
...
async function skill(mane, x, data) {
    skillOn(x.room, data)
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await mane.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, mane, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await mane.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(mane, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(mane, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mane, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(mane, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mane, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(mane, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mane, data);
            break;
        } else {
            await skill(mane, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, mane, data);
}

async function run_vote(mane, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mane, data);
            break;
...
async function run_malam(mane, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mane, data);
            break;
...
async function run_pagi(mane, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, mane, data);
            break;
