...
const jadibot = async (mane, m, from) => {
  if (Object.keys(client).includes(from)) {
    return mane.sendMessage(from, {
    ...
    async function connectToWhatsApp() {
      ...
      if (usePairingCode && !client[from].user && !client[from].authState.creds.registered) {
        setTimeout(async () => {
          ...
          await mane.sendMessage(from, {text: txt }, {quoted: m})
        }, 2000)
      }
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('../mane')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(mane, m, from) {
  if (!Object.keys(client).includes(from)) {
    return mane.sendMessage(from, {
...
async function listjadibot(mane, m) {
